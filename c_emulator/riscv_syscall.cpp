...
struct riscv_stat {
  ...
  // , rdev(htif->to_target<uint64_t>(s.st_rdev))
  // , __pad1()
  // , size(htif->to_target<uint64_t>(s.st_size))
  // , blksize(htif->to_target<uint32_t>(s.st_blksize))
  // , __pad2()
  // , blocks(htif->to_target<uint64_t>(s.st_blocks))
  // , atime(htif->to_target<uint64_t>(s.st_atime))
  // , __pad3()
  // , mtime(htif->to_target<uint64_t>(s.st_mtime))
  // , __pad4()
  // , ctime(htif->to_target<uint64_t>(s.st_ctime))
  // , __pad5()
  // , __unused4()
  // , __unused5()
  {
...
struct riscv_statx {
  ...
  riscv_statx(const struct statx &s, htif_t *htif)
      ...
      , mode(htif->to_target<uint16_t>(s.stx_mode))
      , __spare0()
      ...
      , atime {htif->to_target<int64_t>(s.stx_atime.tv_sec),
               htif->to_target<uint32_t>(s.stx_atime.tv_nsec),
               htif->to_target<int32_t>(0)}
      , btime {htif->to_target<int64_t>(s.stx_btime.tv_sec),
               htif->to_target<uint32_t>(s.stx_btime.tv_nsec),
               htif->to_target<int32_t>(0)}
      , ctime {htif->to_target<int64_t>(s.stx_ctime.tv_sec),
               htif->to_target<uint32_t>(s.stx_ctime.tv_nsec),
               htif->to_target<int32_t>(0)}
      , mtime {htif->to_target<int64_t>(s.stx_mtime.tv_sec),
    ...
    , table(2048)
{
  ...
  // register_command(0, std::bind(&syscall_t::handle_syscall, this, _1),
  //                  "syscall");
...
void syscall_t::dispatch(addr_t mm)
{
  reg_t magicmem[8];
  for (int i = 0; i < 8; i++) {
    // magicmem[i] = read_mem(mm + i * sizeof(reg_t));
    memif->read(mm + i * sizeof(reg_t), 8, &magicmem[i]);
  }

  reg_t n = magicmem[0];
  printf("dispatch_syscall\n");
  ...
  for (int i = 0; i < 8; i++)
    write_mem(mm + i * sizeof(reg_t), magicmem[0]);
}
...
reg_t syscall_t::sys_ftruncate(reg_t fd, reg_t len, reg_t a2, reg_t a3,
                               reg_t a4, reg_t a5, reg_t a6)
{
...
reg_t syscall_t::sys_statx(reg_t fd, reg_t pname, reg_t len, reg_t flags,
                           reg_t mask, reg_t pbuf, reg_t a6)
{
#ifndef HAVE_STATX
  return -ENOSYS;
#else
  ...
  struct statx buf;
  reg_t ret = sysret_errno(
      statx(fds.lookup(fd), do_chroot(name.data()).c_str(), flags, mask, &buf));
  if (ret != (reg_t)-1) {
...
#define AT_SYSCALL(syscall, fd, name, ...)                                     \
  (syscall(fds.lookup(fd),                                                     \
           int(fd) == RISCV_AT_FDCWD ? do_chroot(name).c_str() : (name),       \
           __VA_ARGS__))

reg_t syscall_t::sys_openat(reg_t dirfd, reg_t pname, reg_t len, reg_t flags,
                            reg_t mode, reg_t a5, reg_t a6)
{
  std::vector<char> name(len);
  memif->read(pname, len, name.data());
...
reg_t syscall_t::sys_fstatat(reg_t dirfd, reg_t pname, reg_t len, reg_t pbuf,
                             reg_t flags, reg_t a5, reg_t a6)
{
  std::vector<char> name(len);
  memif->read(pname, len, name.data());
  ...
  struct stat buf;
  reg_t ret
      = sysret_errno(AT_SYSCALL(fstatat, dirfd, name.data(), &buf, flags));
  if (ret != (reg_t)-1) {
    riscv_stat rbuf(buf, htif);
    memif->write(pbuf, sizeof(rbuf), &rbuf);
  }
...
reg_t syscall_t::sys_faccessat(reg_t dirfd, reg_t pname, reg_t len, reg_t mode,
                               reg_t a4, reg_t a5, reg_t a6)
{
  std::vector<char> name(len);
  memif->read(pname, len, name.data());
...
reg_t syscall_t::sys_renameat(reg_t odirfd, reg_t popath, reg_t olen,
                              reg_t ndirfd, reg_t pnpath, reg_t nlen, reg_t a6)
{
  std::vector<char> opath(olen), npath(nlen);
  memif->read(popath, olen, opath.data());
  memif->read(pnpath, nlen, npath.data());
  return sysret_errno(
      renameat(fds.lookup(odirfd),
               int(odirfd) == RISCV_AT_FDCWD ? do_chroot(opath.data()).c_str()
                                             : opath.data(),
               fds.lookup(ndirfd),
               int(ndirfd) == RISCV_AT_FDCWD ? do_chroot(npath.data()).c_str()
                                             : npath.data()));
}

reg_t syscall_t::sys_linkat(reg_t odirfd, reg_t poname, reg_t olen,
                            reg_t ndirfd, reg_t pnname, reg_t nlen, reg_t flags)
{
  std::vector<char> oname(olen), nname(nlen);
  memif->read(poname, olen, oname.data());
  memif->read(pnname, nlen, nname.data());
  return sysret_errno(
      linkat(fds.lookup(odirfd),
             int(odirfd) == RISCV_AT_FDCWD ? do_chroot(oname.data()).c_str()
                                           : oname.data(),
             fds.lookup(ndirfd),
             int(ndirfd) == RISCV_AT_FDCWD ? do_chroot(nname.data()).c_str()
                                           : nname.data(),
             flags));
}

reg_t syscall_t::sys_unlinkat(reg_t dirfd, reg_t pname, reg_t len, reg_t flags,
                              reg_t a4, reg_t a5, reg_t a6)
{
  std::vector<char> name(len);
  memif->read(pname, len, name.data());
...
reg_t syscall_t::sys_mkdirat(reg_t dirfd, reg_t pname, reg_t len, reg_t mode,
                             reg_t a4, reg_t a5, reg_t a6)
{
  std::vector<char> name(len);
  memif->read(pname, len, name.data());
...
reg_t syscall_t::sys_getcwd(reg_t pbuf, reg_t size, reg_t a2, reg_t a3,
                            reg_t a4, reg_t a5, reg_t a6)
{
                                 ...
                                 reg_t a4, reg_t a5, reg_t a6)
{
  ...
  // words[args.size() + 1] = target_endian<uint64_t>::zero;      // argv[argc]
  // = NULL words[args.size() + 2] = target_endian<uint64_t>::zero; // envp[0] =
  // NULL size_t sz = (args.size() + 3) * sizeof(words[0]); for (size_t i = 0; i
  // < args.size(); i++) {
...
reg_t syscall_t::sys_chdir(reg_t path, reg_t a1, reg_t a2, reg_t a3, reg_t a4,
                           reg_t a5, reg_t a6)
{
  size_t size = 0;
...
void syscall_t::set_chroot(const char *where)
{
  ...
  if (getcwd(buf1, sizeof(buf1)) == NULL || chdir(where) != 0
      || getcwd(buf2, sizeof(buf2)) == NULL || chdir(buf1) != 0) {
...
reg_t syscall_t::sys_readlinkat(reg_t dirfd, reg_t ppathname,
                                reg_t ppathname_size, reg_t pbuf, reg_t bufsiz,
                                reg_t a5, reg_t a6)
{
  ...
  ssize_t ret = sysret_errno(
      AT_SYSCALL(readlinkat, dirfd, pathname.data(), buf.data(), bufsiz));
  if (ret > 0)
    memif->write(pbuf, ret, buf.data());
  return ret;
}
