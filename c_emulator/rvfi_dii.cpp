...
void rvfi_handler::send_trace(bool config_print)
{
  ...
  if (trace_version == 1) {
    auto pkt = rvfi_get_exec_packet_v1();
    send_packet(&pkt, config_print);
  } else if (trace_version == 2) {
    auto pkt = rvfi_get_exec_packet_v2();
    send_packet(&pkt, config_print);
    if (rvfi_int_data_present) {
...
rvfi_prestep_t rvfi_handler::pre_step(bool config_print)
{
  ...
  case 0: { /* EndOfTrace */
    ...
    if (insn == (('V' << 24) | ('E' << 16) | ('R' << 8) | 'S')) {
      ...
      auto pkt = RVFI_DII_Execution_Packet_V1::rvfi_get_v2_support_packet();
      send_packet(&pkt, config_print);
...
void rvfi_handler::rvfi_write(uint64_t paddr, uint64_t width, lbits value)
{
