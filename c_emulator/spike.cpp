...
void spike::init_elf(const char *elf_file, uint64_t expected_entry)
{
  ...
  if (elf_symbols.count("tohost") && elf_symbols.count("fromhost")) {
    ...
    fromhost_addr = elf_symbols["fromhost"];
  }
...
{
  uint64_t sail_val = zread_CSR(csr).bits & reg_mask;
...
void spike::setup_model()
{
  ...
  zmip = zMk_Minterrupts({64, get_csr_unchecked(CSR_MIP)});

  // Match the default Spike PMP config, which only sets the first
  // entry to permit unprivileged access to all memory.
  zresultzIbzCuzK a;
  zwrite_CSR(&a, CSR_PMPADDR0, {64, get_csr_unchecked(CSR_PMPADDR0)});
  zwrite_CSR(&a, CSR_PMPCFG0, {64, get_csr_unchecked(CSR_PMPCFG0)});
}
...
{
  uint64_t sail_pc = zget_arch_pc(UNIT).bits;
  uint64_t sail_next_pc = zget_next_pc(UNIT).bits;
  fprintf(stderr, "sail_pc: %lx sail_next_pc: %lx\n", sail_pc, sail_next_pc);
  //  sail: pc_write.value(), spike hart->get_state()->pc
  // TODO: Vector regs.
  bool chk = check_regs() && check_pc() && check_xregs() && check_csrs()
      && check_fregs() && check_time();
...
void spike::mem_write_callback(const char *, sbits paddr, uint64_t width,
                               lbits value)
{
  mem_writes.push_back({paddr.bits, width, value});
}

void spike::mem_read_callback(const char *, sbits paddr, uint64_t width,
                              lbits value)
{
  mem_reads.push_back({paddr.bits, width, value});
}

void spike::mem_exception_callback(sbits, uint64_t) { }

void spike::xreg_full_write_callback(const_sail_string, sbits reg,
                                     sbits value)
{
  // TODO: check whether reg is assigned already.
  xreg_writes[(unsigned)reg.bits] = value.bits;
}

void spike::freg_write_callback(unsigned reg, sbits value)
{
  // TODO: check whether reg is assigned already.
  freg_writes[reg] = value.bits;
}
...
void spike::pc_write_callback(sbits value)
{
  pc_write = value.bits;
}
...
/* Private API */
bool spike::check_regs()
{
  // check x1-x31
  for (int i = 1; i < 32; i++) {
    uint64_t sail_val = zrX(i).bits;
    uint64_t spike_val = hart->get_state()->XPR[i];
    if (sail_val != spike_val) {
      printf("X%d: %lx %c= %lx\n", i, spike_val,
             spike_val == sail_val ? '=' : '!', sail_val);
      return false;
    }
  }
  return true;
...
{
  printf("PC: %lx %c= %lx\n", hart->get_state()->pc,
         hart->get_state()->pc == pc_write.value() ? '=' : '!',
         zget_arch_pc(UNIT).bits);
...
bool spike::check_xregs()
{
  ...
  for (auto write : xreg_writes) {
    ...
    fprintf(stderr,
            "GPR[0x%0x] Sail:0x%0" PRIx64 " Spike:0x%0" PRIx64
            " (xor: 0x%0" PRIx64 ")\n",
            write.first, sail_val, spike_val, sail_val ^ spike_val);
    res &= chk;
  }
...
bool spike::check_time()
{
  ...
  res &= check_csr_vals("MIP", zmip.zbits.bits, get_csr_unchecked(CSR_MIP));
