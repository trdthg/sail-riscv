...
void sail_memif_t::read(addr_t addr, size_t nbytes, void *bytes)
{
  chunked_memif_t::read_chunk(addr, nbytes, bytes);
}

void sail_memif_t::write(addr_t addr, size_t nbytes, const void *bytes)
{
  chunked_memif_t::write_chunk(addr, nbytes, bytes);
}

uint8_t sail_memif_t::read_uint8(addr_t addr)
{
  uint8_t res = 0;
  read(addr, 1, &res);
  return res;
...
void sail_memif_t::write_uint8(addr_t addr, uint8_t val)
{
  write(addr, 1, &val);
}